...
public extension ARSkeleton3D {
    ...
    // TODO extend from ARSkeleton
    
    var jointLocalTransformsAsArray: [[Float]] {
        var ret: [[Float]] = []
        self.jointLocalTransforms.forEach { t in
            let cols = t.columns
            ret.append([cols.0.x, cols.0.y, cols.0.z, cols.0.w,
    ...
    var jointModelTransformsAsArray: [[Float]] {
        var ret: [[Float]] = []
        self.jointModelTransforms.forEach { t in
            let cols = t.columns
            ret.append([cols.0.x, cols.0.y, cols.0.z, cols.0.w,
                cols.1.x, cols.1.y, cols.1.z, cols.1.w,
                cols.2.x, cols.2.y, cols.2.z, cols.2.w,
