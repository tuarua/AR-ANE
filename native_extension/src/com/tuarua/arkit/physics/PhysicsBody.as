...
public class PhysicsBody {
    ...
    public function applyForce(direction:Vector3D, asImpulse:Boolean, at:Vector3D = null):void {
        if (nodeName) {
            var ret:* = ARANEContext.context.call("physics_applyForce", direction, asImpulse, at, nodeName);
    ...
    public function applyTorque(torque:Vector3D, asImpulse:Boolean):void {
        if (nodeName) {
            var ret:* = ARANEContext.context.call("physics_applyTorque", torque, asImpulse, nodeName);
            if (ret is ANEError) throw ret as ANEError;
        }
    }
    ...
    public function clearAllForces():void {
        if (nodeName) {
            var ret:* = ARANEContext.context.call("physics_clearAllForces", nodeName);
            if (ret is ANEError) throw ret as ANEError;
        }
    }
    ...
    public function resetTransform():void {
        if (nodeName) {
            var ret:* = ARANEContext.context.call("physics_resetTransform", nodeName);
            if (ret is ANEError) throw ret as ANEError;
        }
    }
    ...
    public function setResting(resting:Boolean):void {
        if (nodeName) {
            var ret:* = ARANEContext.context.call("physics_setResting", resting, nodeName);
            if (ret is ANEError) throw ret as ANEError;
        }
    }
