...
public class Node extends NodeReference {
    ...
    public function removeChildNodes():void {
        var ret:* = ARANEContext.context.call("node_removeChildren", _name);
    ...
    public function removeFromParentNode():void {
        var ret:* = ARANEContext.context.call("node_removeFromParentNode", _name);
        if (ret is ANEError) throw ret as ANEError;
    ...
    public function addChildNode(node:Node):void {
        ...
        if (_isAdded || _name == "sceneRoot") {
            var ret:* = ARANEContext.context.call("node_addChildNode", _name, node);
            if (ret is ANEError) throw ret as ANEError;
        }
        node.isAdded = true;
    ...
    private function setANEvalue(name:String, value:*):void {
        if (_isAdded) {
            var ret:* = ARANEContext.context.call("node_setProp", _name, name, value);
            if (ret is ANEError) throw ret as ANEError;
        }
    }
    ...
    public function childNode(nodeName:String, recursively:Boolean = true):Node {
        ...
        var ret:* = ARANEContext.context.call("node_childNode", _name, nodeName, recursively);
        //if (theRet is ANEError) throw theRet as ANEError;
        if (ret is ANEError) return null;
        var returnNode:Node = ret as Node;
        if (returnNode) {
